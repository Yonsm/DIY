...
1. 把 `diy`、`ft` 、`boot` 和 3.3.6_0099 固件 `tf_recovery.img` 拷贝到 TF 卡，并插入到摄像机中，重启设备，并等待降级完成；
2. `【可选】` 使用 root/空密码登录 `telnet camera_ip`，此时也可以重启设备一次（从 ft_boot 切换到普通 boot）；
3. `【可选】` 升级至最新固件，已知 3.5.8_0188 可以正常使用，但 RTSP 功能无法正常工作；
4. `【可选】` 此时可以删除 `ft` 、`boot` 和 `tf_recovery.img.bak`；
5. 访问 `http://camera_ip`。
