...
[ -z $WING_HOST] && WING_HOST=192.168.1.9
[ -z $WING_PORT] && WING_PORT=1080

# SSH
scp ../../../PrivY/SSH/authorized_keys $HOST:/etc/storage/ || echo "Usage: $0 [HOST] [NAME] [SSID]"
...
if [ "$HOST" = "ROUTER" ] || [ "$HOST" = "Router" ] || [ "$HOST" = "router" ]; then
	...
	# Contains https/dnsmasq/scripts in storage
	copy_files storage
